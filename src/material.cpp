...
Material::Material(const std::string &vert_path, const std::string &frag_path, bool persist_between_scenes,
    ...
    : Material(vert_path, frag_path, NULL, NULL, persist_between_scenes, vulkan_handler) {};
...
Material::Material(const std::string &vert_path, const std::string &frag_path, std::shared_ptr<Texture> texture,
    ...
    : Material(vert_path, frag_path, texture->getTextureImageView(), texture->getTextureSampler(), persist_between_scenes, vulkan_handler) {
      };
...
Material::Material(const std::string &vert_path, const std::string &frag_path, VkImageView textureImageView, VkSampler textureImageSampler,
  ...
  m_uniform_buffer =
      std::make_unique<Buffer>(m_vulkan_handler->getLogicalDevice(), m_vulkan_handler->getPhysicalDevice(),
                               m_vulkan_handler->getAlignedSize() * MAX_MODELS_PER_MATERIAL, VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT);

  // Create descriptor set
  m_descriptor_set =
      m_vulkan_handler->allocateDescriptorSet(m_uniform_buffer, textureImageView, textureImageSampler, persist_between_scenes);
}
