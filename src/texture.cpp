...
VkImage Texture::createTextureImage(const std::string& path) {
  ...
  result = vkCreateImage(s_vulkan_handler.getLogicalDevice(), &image_create_info, nullptr, &texture_image);
  Utils::checkVkResult(result, "failed to create image!");

  // Get the memory requirements from the runtime
  VkMemoryRequirements memory_requirements;
  vkGetImageMemoryRequirements(s_vulkan_handler.getLogicalDevice(), texture_image, &memory_requirements);
  ...
  result = vkAllocateMemory(s_vulkan_handler.getLogicalDevice(), &allocate_info, nullptr, &texture_image_memory);
  Utils::checkVkResult(result, "failed to allocate image memory!");

  result = vkBindImageMemory(s_vulkan_handler.getLogicalDevice(), texture_image, texture_image_memory, 0) ;
...
void Texture::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout old_layout, VkImageLayout new_layout) {
  VkCommandBuffer command_buffer = s_vulkan_handler.beginSingleTimeCommands();
  ...
  s_vulkan_handler.endSingleTimeCommands(command_buffer);
}
...
void Texture::createTextureImageView(VkImage image) {
  ...
  result = vkCreateImageView(s_vulkan_handler.getLogicalDevice(), &image_view_create_info, nullptr, &m_texture_image_view);
...
void Texture::createTextureSampler() {
  ...
  result = vkCreateSampler(s_vulkan_handler.getLogicalDevice(), &sampler_create_info, nullptr, &m_texture_sampler);
